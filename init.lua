...
local success, commitdata = pcall(function()
    local commitinfo = httpService:JSONDecode(game:HttpGet('https://api.github.com/repos/acqust/CatV5/commits'))[1]
    if commitinfo and type(commitinfo) == 'table' then
        local fullinfo = httpService:JSONDecode(game:HttpGet('https://api.github.com/repos/acqust/CatV5/commits/'.. commitinfo.sha))
...
local function downloadFile(path, func)
	if not isfile(path) then
		local suc, res = pcall(function()
			...
			return game:HttpGet('https://raw.githubusercontent.com/acqust/CatV5/'..readfile('catrewrite/profiles/commit.txt')..'/'..subbed, true)
		end)
...
if not isfolder('catrewrite') or #listfiles('catrewrite') <= 6 or not isfolder('catrewrite/profiles') or not isfile('catrewrite/profiles/commit.txt') then
    ...
    task.spawn(function()
        local req = httpService:JSONDecode(game:HttpGet('https://api.github.com/repos/acqust/CatV5/contents/translations'))
        for _, v in req do
            downloadFile(`catrewrite/{v.path}`)
        end
    end)
